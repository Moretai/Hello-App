...
export default class OrderConfirm extends React.PureComponent {
  ...
  goPay() {
    ...
    const beginTimeStamp = String(year)
    const endTimeStamp = begin
  ...
  render() {
    ...
    // '12.30-13.00' 前面 12.30
    // const chooseTimeStamp = Number(new Date(timeStampToString(String(Number(year)))).getTime())
    // const chooseTimeStamp = String(year)
    // let disabled = false
    // const testB = Number(new Date().getTime() - 1000 * 60 * 60 * Number(label))

    // if (chooseTimeStamp < Number(new Date().getTime() - 1000 * 60 * 60 * Number(label))) {
    //   disabled = true
    // }
    ...
    return (
      <View style={styles.wrap}>
        ...
        <View style={styles.btmBar}>
          ...
          <TouchableOpacity
            style={[styles.rightBar, !chooseAlipay && styles.disabledBtn]}
            onPress={this.goPay}
            disabled={!chooseAlipay}
            >
